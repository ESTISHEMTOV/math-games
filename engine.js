...
/* מתקן את ההקראה כך שתישמע טבעית:
   • מספרים עם מקף-חיבור -> מילים ("מ-90" -> "מתשעים"), במקום מקף שנקרא כהפסקה
   • סימן ° -> "מעלות"
   • מקף עברי (־) בין מילים -> רווח, כדי ש"שווה־שוקיים" ייקרא ברצף
   • שמות-תואר ברבים שהקול משבש (חדות/קהות/ישרות) -> מנוסחים לפי ההגדרה
   הטקסט המוצג על המסך לא משתנה — רק מה שנאמר בקול. */
function forSpeech(t) {
  return String(t)
    ...
    .replace(/°/g, " מעלות")
    .replace(/־/g, " ")
    .replace(/חדות/g, "קטנות מתשעים מעלות")
    .replace(/קהות/g, "גדולות מתשעים מעלות")
    .replace(/ישרות/g, "של תשעים מעלות")
